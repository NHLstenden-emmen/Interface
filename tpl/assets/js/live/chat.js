...
function scrollTableLoad() {
	$(".chatStyle")
		.stop()
		.animate({ scrollTop: $(".chatStyle")[0].scrollHeight }, 700);
}
...
function showMessage(message, id, username) {
	...
	if (localStorage.getItem("team") != "undefined" && username != "Server") {
		...
		messageUsername.innerHTML =
			'<i class="fas fa-wrench" style="color: var(--primaryColor);"></i>&nbsp&nbsp' +
			username +
			" [MOD]";
	} else {
		messageUsername.innerHTML =
			'<i class="avatar fas fa-user-circle"></i>&nbsp&nbsp' + username;
	}
...
function showPollResult(messageData) {
	...
	sortByValue(messageData.result).forEach((element) =>
		pElement.appendChild(
			document.createTextNode(
				element[0] + " heeft " + element[1] + " stemmen "
			)
		)
	);
...
function showPoll(messageData) {
	...
	for (var option in messageData.options) {
		...
		buttonElement.setAttribute(
			"onclick",
			"javascript:sendPoll('" + optionsList[option] + "')"
		);
...
function launchLiveChat(user_idInput) {
	if ("WebSocket" in window) {
		...
		livechatWebSocket.onopen = function () {
			console.log(
				"Connection to server with url : " + livechatWebSocket.url
			);
			livechatWebSocket.send(
				JSON.stringify({ user_id: user_id, type: "join" })
			);
		};
