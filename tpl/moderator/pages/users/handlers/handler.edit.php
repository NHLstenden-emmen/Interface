<?php 
    $gebruikerResult = $DB->Select("SELECT * FROM users WHERE UserID = ? LIMIT 1 ", [ModeratorPageAction])[0];
    ...
    for( $i=0; $i <= 4; $i++) $rankResult .= '<option ' . ($gebruikerResult['Level'] == $i ? 'selected="selected"' : '') . ' value="'.$i.'">' . $user->userLevelName($i) . '</option>';
